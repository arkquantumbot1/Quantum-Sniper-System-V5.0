...
class IEventDispatcher(ABC):
    ...
    @abstractmethod
    def register_conditional_handler(self, condition: Callable[[Event], bool], 
...
class IConfigManager(ABC):
    ...
    _metadata = InterfaceMetadata(
        ...
        created_date=datetime.now(),
        performance_targets={
            "config_load_time": 0.01,
            "config_validation_time": 0.005
        }
    )
...
class IMarketAnalyzer(ABC):
    ...
    @abstractmethod
    def predict_regime_shift(self, data: Dict[str, Any]) -> Tuple[MarketRegime, float, int]:
        """预测状态转换 - 新增极致优化"""
        pass
...
__all__ = [
    # 核心接口
    'IStrategySignal', 'IDataProcessor', 'IEventDispatcher',
    'IConfigManager', 'IMarketAnalyzer', 'IRiskManager',
    
    # 枚举类型
    'SignalDirection', 'SignalPriority', 'DataQualityLevel',
    'EventPriority', 'ConfigScope', 'MarketRegime', 'RiskLevel',
    
    # 数据类
    'PerformanceMetrics', 'InterfaceMetadata', 'SignalMetadata',
    'DataProcessingMetrics', 'Event', 'ConfigChange', 'MarketAnalysis', 'RiskAssessment',
...
__all__.extend([
    ...
    'ExecutionReport', 'MarketData', 'LiquidityProvider'
])
...
# 更新导出列表
__all__.extend([
    'PositionData', 'IOrderExecutor', 'IStrategyEngine'
])
