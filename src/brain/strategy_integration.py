...
class StrategyIntegrationEngine(BaseStrategy):
    ...
    def __init__(
        self, name: str = "StrategyIntegrationEngine", config: Dict[str, Any] = None
    ):
        # ==================== 配置处理 - 稳定版本逻辑 ====================
        config = config or {}
        default_config = {
    ...
    def _initialize_critical_attributes(self):
        ...
        try:
            # 确保性能指标对象存在
            if (
                not hasattr(self, "_performance_metrics")
                or self._performance_metrics is None
            ):
    ...
    def get_status(self) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                # 市场状态
                if (
                    hasattr(self, "current_market_regime")
                    and self.current_market_regime
                ):
                    final_status["market_regime"] = self.current_market_regime.value

                # 高级性能指标
                if hasattr(self, "integration_metrics"):
                    final_status["diversity_index"] = getattr(
                        self.integration_metrics, "diversity_index", 0.0
                    )
                    final_status["integration_latency"] = getattr(
                        self.integration_metrics, "integration_latency", 0.0
                    )
        ...
        try:
            ...
            for strategy_name in strategies_to_load:
                try:
                    ...
                    if (
                        strategy_instance
                        and hasattr(strategy_instance, "initialized")
                        and strategy_instance.initialized
                    ):
                        self.strategies[strategy_name] = strategy_instance
                        self.strategy_weights[strategy_name] = 1.0 / len(
                            strategies_to_load
                        )
                        loaded_count += 1
    ...
    def get_signal(self, data: Any) -> Optional[IStrategySignal]:
        ...
        try:
            ...
            self.logger.info(
                f"策略整合完成: {len(strategy_signals)} 个信号, 共识度: {fusion_result.consensus_score:.3f}"
            )
    ...
    def get_strategy_insights(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # 性能摘要
            if (
                hasattr(self, "strategy_performance")
                and self.strategy_performance
                and insights["strategy_analysis"]
            ):
                performance_scores = [
    ...
    def _initialize_weights(self) -> bool:
        ...
        try:
            ...
            self.logger.info(
                f"权重初始化完成: {len(self.strategies)} 个策略, 初始权重: {initial_weight:.3f}"
            )
            return True
    ...
    def _collect_strategy_signals(self, data: Any) -> Dict[str, IStrategySignal]:
        ...
        for strategy_name, strategy in self.strategies.items():
            try:
                ...
                if (
                    signal
                    and hasattr(signal, "get_confidence")
                    and signal.get_confidence()
            ...
            if strategy_name in self.strategy_performance:
                ...
                if hasattr(signal, "get_confidence"):
                    performance.success_rate = (
                        performance.success_rate * 0.9 + signal.get_confidence() * 0.1
                    )
    ...
    def _dynamic_reweight_strategies(self):
        ...
        try:
            ...
            if total_performance > 0:
                for strategy_name, performance in self.strategy_performance.items():
                    new_weight = max(
                        0.01, performance.performance_score / total_performance
                    )
    ...
    def _update_integration_metrics(
        self, fusion_result: FusionResult, integration_time: float
    ):
        """更新整合指标 - 完整版本特性"""
        ...
        if hasattr(fusion_result.final_signal, "get_confidence"):
            self.integration_metrics.average_confidence = (
                fusion_result.final_signal.get_confidence()
            )
    ...
    def _get_ultimate_safe_integration_info(self) -> Dict[str, Any]:
        ...
        try:
            ...
            total_strategies = (
                len(strategies)
                if strategies is not None and hasattr(strategies, "__len__")
                else 0
            )

            # 安全获取活跃策略数
            integration_metrics = self._get_ultimate_safe_attribute(
            ...
            quantum_coherence = self._get_ultimate_safe_attribute(
                "quantum_coherence_level", 1.0
            )

            market_regime = "unknown"
            current_regime = self._get_ultimate_safe_attribute(
