...
class StrategyEngine(BaseStrategy):
    ...
    def __init__(
        self, name: str = "QuantumStrategyEngine", config: Dict[str, Any] = None
    ):
        # 配置处理 - 整合版本
        config = config or {}
        default_config = {
        ...
        self._execution_mode: ExecutionMode = ExecutionMode(
            config.get("execution_mode", "realtime")
        )
        ...
        self._thread_pool = ThreadPoolExecutor(
            max_workers=config.get("max_concurrent_strategies", 10)
        )
    ...
    def get_signal(self, data: Any) -> Optional[IStrategySignal]:
        ...
        try:
            ...
            # 验证和优化最终信号
            final_signal = self._validate_and_optimize_signal(
                integrated_signal, strategy_context
            )
            if not final_signal:
                return None
            ...
            class EmergencyBaseStrategy(BaseStrategy):
                ...
                def get_signal(self, data):
                    ...
                    metadata = SignalMetadata(
                        source="emergency_base_strategy", tags=["emergency", "stable"]
                    )
    ...
    def _integrate_signals_simple(
        self, signals: Dict[str, IStrategySignal]
    ) -> Optional[IStrategySignal]:
        """简单信号整合 - 应急版本稳定实现"""
        try:
            # 简单整合：选择置信度最高的信号
            best_signal = None
    ...
    def _initialize_integration_engine(self) -> bool:
        ...
        try:
            ...
            for strategy_name, strategy_instance in self._strategies.items():
                ...
                self._integration_engine.add_strategy(
                    strategy_name, strategy_instance, initial_weight
                )
    ...
    def start_engine(self) -> bool:
        ...
        try:
            with self._engine_lock:
                ...
                self.logger.info(
                    f"策略引擎启动完成: {success_count}/{len(self._strategies)} 个策略"
                )
    ...
    def _publish_signal_event(
        self, signal: IStrategySignal, strategy_count: int, processing_time: float
    ):
        """发布信号事件 - 条件实现"""
        if not self._enable_advanced_features:
            return
    ...
    def _update_signal_metrics(self, processing_time, signal_count):
        ...
        try:
            ...
            self._engine_metrics.average_signal_latency = (
                self._engine_metrics.average_signal_latency * 0.9
                + processing_time * 0.1
            )
    ...
    def _get_performance_metrics_dict(self):
        """安全获取性能指标字典"""
        try:
            if hasattr(self, "_performance_metrics") and hasattr(
                self._performance_metrics, "to_dict"
            ):
