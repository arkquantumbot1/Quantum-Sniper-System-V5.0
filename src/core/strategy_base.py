...
class StrategySignal(IStrategySignal):
    ...
    def __str__(self) -> str:
        return (
            f"StrategySignal(type={self._signal_type}, "
...
class BaseStrategy(ABC):
    ...
    def optimize_strategy(self) -> bool:
        ...
        try:
            ...
            self.logger.info(
                f"策略 {self.name} 优化完成，性能分数: {self._status['performance_score']:.3f}"
            )
            return True
    ...
    def __repr__(self) -> str:
        return (
            f"BaseStrategy(name={self.name}, initialized={self.initialized}, "
...
__all__ = ["BaseStrategy", "StrategySignal", "StrategyError", "StrategyFactory"]
