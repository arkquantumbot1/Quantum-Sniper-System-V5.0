...
class BaseConfigManager(IConfigManager):
    ...
    def _update_metadata(self):
        ...
        if self.config_path:
            ...
            self._metadata_info.checksum = hashlib.md5(config_content.encode()).hexdigest()
    ...
    def _detect_config_changes(self, old_config: Dict, new_config: Dict) -> List[ConfigChange]:
        """检测配置变更 - 内部方法"""
        changes = []
            ...
            if isinstance(data, dict):
                for key, value in data.items():
                    ...
                    structure[key] = {
                        ...
                        "path": current_path,
                        "children": analyze_structure(value, current_path) if isinstance(value, dict) else None
                    }
            return structure
    ...
    def __repr__(self) -> str:
        return (f"BaseConfigManager(scope={self.scope.value}, "
...
class ConfigManagerFactory:
    ...
    @classmethod
    def create_manager(cls, scope: ConfigScope, config_path: str = None) -> BaseConfigManager:
        """创建配置管理器"""
        ...
        for interface_name in InterfaceRegistry.list_interfaces():
            ...
            if (interface_class and 
                hasattr(interface_class, '_metadata') and
                "config" in interface_class._metadata.description.lower() and
                issubclass(interface_class, BaseConfigManager)):
